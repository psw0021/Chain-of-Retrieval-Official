...
class Retriever:
    def __init__(self, args, result_folder_path):
        ...
        self.root_dir = os.path.join(root_dir, self.retrieval_method)
        
        os.makedirs(self.root_dir, exist_ok=True)
        
        self.bm_25_model = None
    ...
    def initialize(self):
        self.format_corpus()
        if self.retrieval_method == "rank-BM25":
            tokenized_candidates = [doc.split() for doc in self.corpus]
            self.bm_25_model = BM25Okapi(tokenized_candidates)
    ...
    def retrieve_top_k(self, query, top_k):        
        tokenized_query = self.encode_query(query)  # Encode the query
        if self.use_chunked == False and self.use_full_paper_as_corpus == False:
            logging.info(f"Retrieving with abstract candidates only")
            #best_docs = self.bm_25_model.get_top_n(tokenized_query, self.corpus, n=top_k)
            scores = self.bm_25_model.get_scores(tokenized_query)
            best_doc_indices = sorted(range(len(scores)), key=lambda i: scores[i], reverse=True)[:top_k]

            top_k_corpus = [(self.corpus[idx], scores[idx]) for i,idx in enumerate(best_doc_indices)]  
        
        elif self.use_chunked == True and self.use_full_paper_as_corpus == False:
            logging.info(f"Retrieving with chunked paper candidates")
            #best_docs = self.bm_25_model.get_top_n(tokenized_query, self.corpus, n=top_k)
            scores = self.bm_25_model.get_scores(tokenized_query)
            best_doc_indices = sorted(range(len(scores)), key=lambda i: scores[i], reverse=True)[:top_k]

            top_k_corpus = [(self.chunked_corpus_to_paper_dict[self.corpus[idx]], scores[idx], self.corpus[idx]) for i,idx in enumerate(best_doc_indices)]           
        
        elif self.use_chunked == False and self.use_full_paper_as_corpus == True:
            logging.info(f"Retrieving with full paper candidates")
            #best_docs = self.bm_25_model.get_top_n(tokenized_query, self.corpus, n=top_k)
            scores = self.bm_25_model.get_scores(tokenized_query)
            best_doc_indices = sorted(range(len(scores)), key=lambda i: scores[i], reverse=True)[:top_k]

            top_k_corpus = [(self.full_paper_corpus_to_paper_dict[self.corpus[idx]], scores[idx], self.corpus[idx]) for i,idx in enumerate(best_doc_indices)]
