...
from Retrieval.metrics import evaluate_retrieval_CSFCube
...
class Retriever:
    ...
    def retrieve_top_k(self, query, top_k, agent_name, current_embedding, target_corpus) -> list:
        """
        ...
        l2_distance_list = []
        for index in range(0, len(current_embedding)):
            document_vector = current_embedding[index]
            if not isinstance(document_vector, torch.Tensor):
                document_vector = torch.from_numpy(document_vector)
                document_vector = document_vector.unsqueeze(0)
            if not isinstance(query_embedding, torch.Tensor):
                query_embedding = torch.from_numpy(query_embedding)
            l2_distance = torch.cdist(query_embedding, document_vector, p=2).item()
            
            l2_distance_list.append((target_corpus[index], l2_distance))
            
        top_k_corpus = sorted(l2_distance_list, key=lambda x: x[1])
        
        top_k_corpus = top_k_corpus[:top_k]
    ...
    def SubTreeExplore(self, args, query_full_paper, query_abstract, parent_name, current_embedding, target_corpus, abstract_to_full_context_dict) -> tuple:
        """
        ...
        for agent_name, current_query in formatted_query:
            if self.only_A2A == False:
                top_k_corpus = self.retrieve_top_k(current_query, args.top_k, agent_name, current_embedding, target_corpus)                
            elif self.only_A2A == True:
                if agent_name == f"{parent_name}_Original Retrieval" and current_query != "":
                    top_k_corpus = self.retrieve_top_k(current_query, args.top_k, "Original Retrieval", current_embedding, target_corpus)
                else:
                    top_k_corpus = []
    ...
    def evaluate(self, args, test_file_directory, result_folder_path):
        ...
        """ 
        total_results = []
        logging.info(len(test_file_directory))
        for files in test_file_directory:
            if self.use_query_optimizer == False:
                ...
                current_embedding = self.build_candidate_embeddings(formatted_target_corpus)
            ...
            if self.use_query_optimizer == False:                   
                if self.iterative_retrieval == False:
                    ...
                    top_k_corpus = self.retrieve_top_k(formatted_query, args.top_k, agent_name, current_embedding, formatted_target_corpus)
                    ...
                    for agent_name, current_query in formatted_query:
                        ...
                        top_k_corpus = self.retrieve_top_k(current_query, args.top_k, agent_name, current_embedding, formatted_target_corpus)
                        organized_retrieved_top_corpus_list.append(top_k_corpus)
                            ...
                            for parent_agent, current_query in formatted_query:
                                if self.only_A2A == False:
                                    if parent_agent != "Original Retrieval":
                                        ...
                                        retrieved_total_corpus_per_agent, retrieved_corpus_per_agent, optimized_queries = self.SubTreeExplore(args, full_paper, abstract, parent_agent, current_embedding, formatted_target_corpus, abstract_to_full_context_dict)
            ...
            current_result = evaluate_retrieval_CSFCube(parsed_retrieved_top_corpus, formatted_correct_candidates, formatted_target_corpus_to_score, args.top_k, args.max_top_k)
            ...
            logging.info(f"{current_result_folder_path}")