...
from Retrieval.retriever_SCIDocs_Full import Retriever
    ...
    parser.add_argument("--embedding_model", default="jina-embeddings-v2-base-en", choices=["SPECTER", "text-embedding-3-small", \
        "jina-embeddings-v2-base-en", "SPECTER2", "SPECTER2_Base", \
            "SciNCL", "text-embedding-ada-002", "bge-m3", "dewey_en_beta", \
               "Qwen3-Embedding-0.6B", "Qwen3-Embedding-8B", "granite-embedding-english-r2", "inf-retriever-v1-1.5b"])
    parser.add_argument("--embedding_fusion", default=False, type=lambda x: x.lower() == "true")
    parser.add_argument("--embedding_fuse_method", default="naive_aggregation", choices=["naive_aggregation", "maximum_similarity", "single_query"])
...
def evaluate(args):
    ...
    result_directory = "Existing_Benchmark_Results/SciDocs_FullPaper_Results"
    ...
    if args.iterative_retrieval == True and args.SubTreeSearch == True:
        if args.use_multi_source == True:
            if args.use_chunked == True and args.use_full_paper_as_corpus == False:
                pass
            else:
                raise TypeError("When using SubTreeSearch with multi-source retrieval, you must always use abstract and chunked source retrieval")
        if args.use_multi_source == False:
            if args.use_chunked == False and args.use_full_paper_as_corpus == False:
                pass
            else:
                raise TypeError('When using SubTreeSearch with single-source retrieval, you must always not use full paper corpus and chunked source retreval')
    ...
    if args.use_single_optimized_query == True:
        if args.use_base_agent == False:
            raise TypeError("When using single query optimizer, you should always use base query optimizer. ")
        if args.use_trained_model == True:
            raise TypeError("When using single query optimizer, you shoiuld always use untrained models. ")
        if args.use_query_optimizer == False:
            raise TypeError("When using single query optimizer, you should always use query optimizer. ")
        if args.embedding_fusion == False:
            raise TypeError("When using single query optimizer, you should use embedding fusion method. ")
        if args.embedding_fuse_method != "single_query":
            raise TypeError("When using single query optimizer, you should use single query embedding fusion method")
        if args.use_multi_source == True:
            raise TypeError("when using single query optimizer, you should evaluate under single source retrieval")
        
    if args.use_abstract_for_query_optimization == True:
        if args.use_trained_model == True:
            raise TypeError("When using abstract input for query optimization, you should always use untrained models")
        if args.use_query_optimizer == False:
            raise TypeError("When using abstract input for query optimization, you should always use query optimizers")
        if args.use_multi_source == True:
            raise TypeError("When using abstract input for query optimization, you should always use single source retrieval")
        if args.use_multi_source == False:
            if args.use_full_paper_as_corpus == True or args.use_chunked == True:
                raise TypeError("When using abstract input for query optimization, you should always use abstract source retrieval")
    ...
    if args.iterative_retrieval == False:
        if args.use_query_optimizer == True:
            if args.embedding_fusion == True:
                if args.use_single_optimized_query == False:
                ...
                elif args.use_single_optimized_query == True:
                    result_folder_name = f"{last_two_dirs}/remove_citations_{args.remove_citations}_remove_mentions_{args.remove_mentions}/Use_QueryOptimizers/use_trained_model_{args.use_trained_model}/{args.query_optimizer_model}/use_multi_source_{args.use_multi_source}/multi_agent_{args.multi_agent}_INCLUDE_ORIGINAL_RETRIEVAL_{args.include_original_retrieval}_BASE_{args.use_base_agent}_METHOD_{args.use_method_agent}_RESEARCH_QUESTION_{args.use_research_question_agent}_EXPERIMENT_{args.use_experiment_agent}_SINGLE_QUERY_OPTIMIZER_{args.use_single_optimized_query}/{args.embedding_model}_top_{args.top_k}_use_chunked_{args.chunk_unit}_{args.use_chunked}_use_full_paper_as_corpus_{args.use_full_paper_as_corpus}_fuse_embedding_{args.embedding_fusion}_{args.embedding_fuse_method}/iteration_{args.iteration_num}"
                ...
                if args.use_trained_model == False:
                    if args.use_abstract_for_query_optimization == False:
                        ...
                        result_folder_name =  f"{last_two_dirs}/remove_citations_{args.remove_citations}_remove_mentions_{args.remove_mentions}/Use_QueryOptimizers_w_abstract_input_{args.use_abstract_for_query_optimization}/use_trained_model_{args.use_trained_model}/{args.query_optimizer_model}/use_multi_source_{args.use_multi_source}/multi_agent_{args.multi_agent}_INCLUDE_ORIGINAL_RETRIEVAL_{args.include_original_retrieval}_BASE_{args.use_base_agent}_METHOD_{args.use_method_agent}_RESEARCH_QUESTION_{args.use_research_question_agent}_EXPERIMENT_{args.use_experiment_agent}/{args.embedding_model}_top_{args.top_k}_use_chunked_{args.chunk_unit}_{args.use_chunked}_use_full_paper_as_corpus_{args.use_full_paper_as_corpus}/iteration_{args.iteration_num}"
                        ...
                        if args.starting_iteration_of_A2A_only > 0:
                            if args.use_aspect_aware_cache_for_selection == True:
                                if args.use_abstract_for_query_optimization == False:
                                    ...
                                    result_folder_name = f"{last_two_dirs}/remove_citations_{args.remove_citations}_remove_mentions_{args.remove_mentions}/Use_QueryOptimizers_w_abstract_input_{args.use_abstract_for_query_optimization}/iterative_retrieval_{args.iterative_retrieval}_total_iterations_{args.total_iterative_retrieval_loop}_SubTreeSearch_{args.SubTreeSearch}_Recursive_Merge_{args.Recursive_Merge}_selector_starting_idx_{args.selector_starting_idx}_starting_iteration_of_A2A_only_{args.starting_iteration_of_A2A_only}/use_trained_model_{args.use_trained_model}/{args.query_optimizer_model}/use_multi_source_{args.use_multi_source}/multi_agent_{args.multi_agent}_INCLUDE_ORIGINAL_RETRIEVAL_{args.include_original_retrieval}_BASE_{args.use_base_agent}_METHOD_{args.use_method_agent}_RESEARCH_QUESTION_{args.use_research_question_agent}_EXPERIMENT_{args.use_experiment_agent}/{args.embedding_model}_top_{args.top_k}_use_chunked_{args.chunk_unit}_{args.use_chunked}_use_full_paper_as_corpus_{args.use_full_paper_as_corpus}/iteration_{args.iteration_num}"
