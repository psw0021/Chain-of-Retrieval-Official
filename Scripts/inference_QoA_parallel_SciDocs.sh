...
use_abstract_for_query_optimization=True
gpu_memory_utilization=0.7
use_gpt=True
## Set the benchmark directory for SciFullBench, as shown below
benchmark_directory="Existing_Benchmarks/SciDocs/scidocs_cocite"
...
function run_job() {
    ...
    local job_num=$3
    local iteration=$4
    echo "Running inference_retrieval_SciDocs.py"
    rm logs/logs_SciDocs/output_${job_num}.log 
    python Inference/inference_retrieval_SciDocs.py \
    ...
    --selector_starting_idx ${selector_starting_idx} > logs/logs_SciDocs/output_${job_num}.log 2>&1
}
