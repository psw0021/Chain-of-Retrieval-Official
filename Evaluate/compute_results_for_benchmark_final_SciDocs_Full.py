...
evaluation_directory_template = ""

print(f"Currently evaluating embedding model with {embedding_model}")        
print(evaluation_directory_template)        
total_test_sets = 0
for iteration in range(1, 4):
    ...
    benchmark_split = ["scidocs_view", "scidocs_read", "scidocs_cocite"]
    for split in benchmark_split:
        evaluation_directory = evaluation_directory_template.format(split=split, embedding_model=embedding_model, iteration=iteration)
        file_list = traverse_folder(evaluation_directory)
    ...
    print(f"Number of test sets for iteration {iteration} is {number_of_test_sets}")
